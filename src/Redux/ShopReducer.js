    ...
    products: [
        {
            ...
            title: "Watch",
            description:
              `This watch is not just a sight to behold but also comes equipped with innovative features
...
const ShopReducer = (state=initialState, action) => {
    switch(action.type) {
        ...
        case Actions.OPEN_CURRENT_ITEM:
            return {
                ...state,
        ...
        case Actions.UPDATE_QTY:
            return {
                ...state,
                cart: state.cart.map((product) => product.id == action.payload.id ? {...product, qty: action.payload.qty} : 
                product)
            }
